---
title: "Workflow of scATAC-seq with multiome - Normalization and dimentionallity reduction"
author: 
...
Hi to everyone!
In this notebook we are going to practice how to analise scATAC-se using multiome data (ATAC + GEX) by using the `Signac`( _Stuart et al._ , 2020) package. `Signac` ( _Stuart et al._ , 2020) in an extention of the very well-known `Seurat` package to analyse scATAC-seq data. We could say that is the successor to sequencing  techniques like ChIP-seq and RRBS-Seq (a type of bisulfite sequencing). 


In this report we will present Signac by analyzing a data set composed of 3 multiome samples
which have in average ---- cells from the human tonsil which belong to the Tonsil Atlas article (Massoni R.et al. 2022)
