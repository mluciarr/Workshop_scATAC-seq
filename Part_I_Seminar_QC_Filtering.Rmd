...
Hi to everyone!
In this notebook we are going to practice how to analyse scATAC-se using multiome data (ATAC + GEX) by using the `Signac`(Stuart _et al._ , 2020) package. `Signac` (Stuart _et al._ , 2020) in an extension of the very well-known `Seurat` package to analyse scATAC-seq data. We could say that is the successor to sequencing  techniques like ChIP-seq and RRBS-Seq (a type of bisulfite sequencing). 
